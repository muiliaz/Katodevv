...
Что именно утекало наружу, я проверил по коду — источников `err.message` в этом `catch` три:
...
Проверил все три точки отправки:
...
`throw new Error(data.description)` использует только текст от Telegram. Это я проверил по
...
Прогнал оба handler-а напрямую в Node без `TELEGRAM_TOKEN`/`TELEGRAM_CHAT_ID` в окружении — это
...
### Побочное наблюдение, которое я НЕ стал чинить здесь
...
Не чиню в этом шаге сознательно: это изменение публичного HTTP-контракта, а не сокрытие деталей, и
