...
### Поправка от 2026-08-06: сеть была, я её не проверил

Изначально я оставил actions на плавающих тегах и написал здесь, что «сетевого доступа к GitHub
нет». **Это было неверно.** Я проверил только отсутствие `gh` (`which gh` → not found) и достроил
...
**Обновлять до v5+ не стал.** Актуальные релизы — `actions/checkout` v7.0.1 и `actions/setup-node`
v7.0.0, но аудит просил *закрепить*, а не обновить. Мажорное обновление action меняет требования к
runtime раннера, и проверить его я могу только запуском в GitHub Actions, то есть уже после
...
`.github/dependabot.yml` с экосистемой `github-actions`. Не добавил: этого не просил ни один аудит,
