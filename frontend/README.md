...
| `npm run lint` | ESLint — только правила корректности, стиль за Prettier |
| `npm run format` | Prettier: переформатировать. `check:format` — проверить, **эта команда идёт в CI** |
| `npm run check:css` | Классы в CSS, которые ничем не применяются — **эта команда идёт в CI** |
...
CI (`.github/workflows/ci.yml`) состоит из двух джобов. Первый, `build`, — семь шагов, и их можно
...
| `src/pages/web/Services.test.jsx` | четыре карточки услуг, модалка с демо, переключатель платформ, терминал с ценами, перезапуск демо при смене языка |
| `src/shared/ChatWidget/ChatWidget.test.jsx` | воронка чата целиком: обе ветки, свободный текст, отказ и повтор, восстановление из `sessionStorage` |
...
заявку и проверяет payload и состояние успеха. `e2e/lead-paths.spec.js` — два других пути к лиду
(чат-виджет и модалка тарифа на `/bots`) и переключение EN/RU. Netlify-функция перехватывается, Turnstile
...
| Prettier не трогает два места | `BlackHole.jsx` (GLSL в шаблонных строках — переформатирование делает шейдеры нечитаемыми) и Markdown (падит ячейки таблиц по кодовым точкам, кириллица разъезжается). Оба исключения описаны в `.prettierignore` |
