...
function BotServiceCard({ card, onClick }) {
  ...
  useEffect(() => {
    ...
    return () => {
      if (badgeOpTween.current) badgeOpTween.current.kill();
    };
  }, []);
  ...
  function handleMouseEnter() {
    ...
    if (badgeOpTween.current) {
      badgeOpTween.current.kill();
      badgeOpTween.current = null;
    }
    ...
    badgeEl.style.boxShadow =
      "0 0 32px rgba(255,176,0,0.35), 0 0 64px rgba(255,176,0,0.15), inset 0 1px 0 rgba(255,255,255,0.25)";
  ...
  function handleMouseLeave() {
    ...
    if (badgePulseTween.current) {
      badgePulseTween.current.kill();
      badgePulseTween.current = null;
    }
    const badgeEl = badgeRef.current;
    const badgeTxtEl = badgeEl.querySelector(".popular-badge-text");
  ...
  return (
    <div className="bot-card-outer">
      ...
      >
        ...
        <span ref={iconRef} className="card-icon">
          🤖
        </span>
        <h3>{card.title}</h3>
