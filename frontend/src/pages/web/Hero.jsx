...
function Hero() {
  ...
  useEffect(() => {
    ...
    const tick = (time) => {
      ...
      if (titleH1)
        titleH1.style.transform = `translateY(${pullY.toFixed(3)}px) scale(${pullScale.toFixed(4)})`;
      if (sLineEl)
        sLineEl.style.transform = `translateY(${(pullY * 0.7).toFixed(3)}px) scale(${pullScale.toFixed(4)})`;
      if (btnsEl)
        btnsEl.style.transform = `translateY(${(pullY * 0.5).toFixed(3)}px) scale(${pullScale.toFixed(4)})`;
  ...
  useGSAP(
    () => {
      ...
      tl.fromTo(".hero-service-line", { opacity: 0 }, { opacity: 1, duration: 0.55 }, 2.2);
      tl.fromTo(
        ".hero-v2-buttons > *",
        { opacity: 0 },
      ...
      <div className="hero-v2-inner">
        <div className="hero-v2-center">
          ...
          <div className="hero-service-line" ref={serviceLineRef}>
            ...
            <span>{lang === "ru" ? "БОТЫ · ВЕБ · ПРИЛОЖЕНИЯ · AI" : "BOTS · WEB APPS · AI"}</span>
          </div>
      ...
      {showPicker && (
        <>
          ...
          <div className="hero-picker-panel">
            <button className="hero-picker-close" onClick={() => setShowPicker(false)}>
              ✕
            </button>
            ...
            <div className="hero-picker-grid">
              {SERVICE_INFO.map((svc, i) => (
                ...
                >
                  ...
                  <span className="hero-picker-desc">{svc[lang].desc.split(".")[0]}</span>
                </button>
              ))}
            </div>
          </div>
