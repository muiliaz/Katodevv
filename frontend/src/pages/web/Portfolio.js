...
import { PORTFOLIO_DATA, CATEGORY_KEYS } from "./portfolioData";
...
/* ══════════════════════════════════════════════════
   Demo component map — web category only
══════════════════════════════════════════════════ */
const DEMO_MAP = {
  landing:  LandingExample,
  business: BusinessExample,
  ecom:     EcommerceExample,
  crm:      CRMExample,
};
...
function Portfolio() {
  ...
  const [displayedTab, setDisplayedTab] = useState('web');
  const [active,       setActive]       = useState(null);   // modal id
  ...
  const currentItems  = PORTFOLIO_DATA[displayedTab].items;
  const ActiveExample = active ? DEMO_MAP[active] : null;

  /* ── Escape + Lenis for modal ── */
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (window.__lenis) active ? window.__lenis.stop() : window.__lenis.start();
    return () => { document.body.style.overflow = ''; };
  }, [active]);

  /* ── Initial indicator position ── */
  useEffect(() => {
    const tabEl = tabRefs.current['web'];
    if (tabEl && indicatorRef.current) {
      gsap.set(indicatorRef.current, { x: tabEl.offsetLeft, width: tabEl.offsetWidth });
    }
  }, []);
  ...
  /* ── Animate cards in after displayedTab changes ── */
  useEffect(() => {
    const cards = Array.from(gridRef.current?.querySelectorAll('.portfolio-card') ?? []);
    ...
    if (!reduced) {
      gsap.set(cards, { scale: 0.9, opacity: 0, y: -20, filter: 'blur(8px)' });
      gsap.to(cards, {
        scale: 1, opacity: 1, y: 0, filter: 'blur(0px)',
        duration: 0.5, ease: 'power3.out',
        stagger: mobile ? 0.02 : 0.08,
      });
    } else {
      gsap.set(cards, { opacity: 0 });
      gsap.to(cards, { opacity: 1, duration: 0.3, stagger: 0.05 });
    }
  }, [displayedTab]);
  ...
  function handleTabClick(key) {
    ...
    if (!reduced && cards.length) {
      transitionTL.current = gsap.timeline()
        .to(cards, {
          scale: 0.85, opacity: 0, y: 20, filter: 'blur(8px)',
          duration: 0.35, ease: 'power2.in',
          stagger: mobile ? 0.02 : 0.04,
        })
        .call(() => setDisplayedTab(key), null, '+=0.1');
    } else {
      setDisplayedTab(key);
    }
  }
  ...
  function handleCardClick(item) {
    ...
    // Wiggle + tooltip
    const cardEl = document.querySelector(`[data-card-id="${item.id}"]`);
    if (cardEl) {
      gsap.timeline()
        .to(cardEl, { x: -6, duration: 0.08, ease: 'power2.out' })
        .to(cardEl, { x:  6, duration: 0.08, ease: 'power2.out' })
        .to(cardEl, { x: -4, duration: 0.08, ease: 'power2.out' })
        .to(cardEl, { x:  4, duration: 0.08, ease: 'power2.out' })
        .to(cardEl, { x:  0, duration: 0.06, ease: 'power2.out' });
    }
    ...
    <section className="portfolio">
        ...
        <div className="port-tabs">
          {CATEGORY_KEYS.map((key) => (
            <button
              ...
              className={`port-tab ${activeTab === key ? 'active' : ''}`}
              onClick={() => handleTabClick(key)}
            >
              {t.portfolio.tabs[key]}
            </button>
          ))}
          <div ref={indicatorRef} className="port-tab-indicator" />
        </div>
      </div>
      ...
      <div className="portfolio-grid" ref={gridRef}>
        {currentItems.map((item) => {
          ...
          const tag   = lang === 'ru' ? item.tagRu   : item.tagEn;
          return (
            <div
              key={item.id}
            ...
            >
              {tooltipId === item.id && (
                <div className="port-coming-soon">{t.portfolio.comingSoon}</div>
              )}
              <div className="card-thumb" style={{ background: item.gradient }}>
                <div className="card-thumb-icon">{item.icon}</div>
                <div className="thumb-browser"><span /><span /><span /></div>
              </div>
